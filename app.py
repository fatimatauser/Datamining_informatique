...
def apply_fp_growth(df):
    ...
    try:
        basket = df.groupby([transaction_col, item_col])[item_col].count().unstack().fillna(0)
...
def apply_kmeans(df):
    ...
    with st.expander("Paramètres", expanded=True):
        ...
        num_cols = df.select_dtypes(include='number').columns
        features = st.multiselect("Variables à inclure", num_cols, default=num_cols[:2])
    ...
    st.success(f"{k} clusters créés avec succès!")
    ...
    with c2:
        ...
        if len(features) >= 2:
            ...
            st.warning("Sélectionnez au moins 2 variables pour la visualisation")
