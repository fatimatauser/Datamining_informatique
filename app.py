...
def apply_fp_growth(df):
    ...
    st.markdown("""
    <div class="info-card">
        <b>Conseil :</b> Pour cette analyse, vous avez besoin de deux colonnes :
        <ul>
            <li>Une colonne identifiant les transactions (ex: numéro de commande)</li>
            <li>Une colonne identifiant les produits (ex: nom du produit, SKU)</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Configuration", expanded=True):
        c1, c2 = st.columns(2)
        with c1:
            transaction_col = st.selectbox("Colonne transaction (ex: commande)", df.columns)
        with c2:
            item_col = st.selectbox("Colonne produit (ex: article)", df.columns)
...
def apply_kmeans(df):
    ...
    with st.expander("Paramètres", expanded=True):
        ...
        num_cols = df.select_dtypes(include='number').columns
        if len(num_cols) < 2:
            st.warning("Votre jeu de données ne contient pas suffisamment de colonnes numériques pour cette analyse")
            return
            
        features = st.multiselect("Variables à inclure", num_cols, default=num_cols[:min(2, len(num_cols))])
...
def apply_rfm(df):
    ...
    st.markdown("""
    <div class="info-card">
        <b>Conseil :</b> Pour cette analyse, vous avez besoin de trois types de données :
        <ul>
            <li><b>Récence</b>: Dernière date d'achat ou nombre de jours depuis le dernier achat</li>
            <li><b>Fréquence</b>: Nombre d'achats ou de transactions</li>
            <li><b>Montant</b>: Valeur totale dépensée par le client</li>
        </ul>
        Si vous avez une colonne de date, l'application peut calculer automatiquement la récence.
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.expander("Configuration", expanded=True):
        ...
        cols = st.columns(3)
        with cols[0]:
            recency_options = [col for col in df.columns if 'date' in col.lower() or 'jour' in col.lower() or 'last' in col.lower()]
            recency_col = st.selectbox("Colonne récence (date ou nombre)", 
                                      options=df.columns,
                                      index=df.columns.get_loc(recency_options[0]) if recency_options else 0)
        with cols[1]:
            frequency_options = [col for col in df.columns if 'freq' in col.lower() or 'count' in col.lower() or 'command' in col.lower()]
            frequency_col = st.selectbox("Colonne fréquence (nombre)", 
                                        options=df.columns,
                                        index=df.columns.get_loc(frequency_options[0]) if frequency_options else 0)
        with cols[2]:
            monetary_options = [col for col in df.columns if 'montant' in col.lower() or 'amount' in col.lower() or 'total' in col.lower() or 'revenue' in col.lower()]
            monetary_col = st.selectbox("Colonne montant (valeur)", 
                                      options=df.columns,
                                      index=df.columns.get_loc(monetary_options[0]) if monetary_options else 0)
        
        st.subheader("Options de calcul")
        calculate_recency = st.checkbox("Calculer la récence à partir d'une date", value=True)
        reference_date = st.date_input("Date de référence pour le calcul de la récence", value=datetime.today())
    ...
    try:
        ...
        for col in ['Frequence', 'Montant']:
            rfm[col] = pd.to_numeric(rfm[col], errors='coerce')
        
        # Gestion spéciale de la récence
        if calculate_recency:
            # Convertir en datetime si possible
            try:
                rfm['Recence'] = pd.to_datetime(rfm['Recence'])
                # Calculer le nombre de jours depuis la date de référence
                rfm['Recence'] = (pd.to_datetime(reference_date) - rfm['Recence']).dt.days
                st.success(f"Récence calculée : nombre de jours depuis {reference_date}")
            except Exception as e:
                st.error(f"Erreur de conversion de date : {str(e)}. Utilisation des valeurs brutes.")
                rfm['Recence'] = pd.to_numeric(rfm['Recence'], errors='coerce')
        else:
            rfm['Recence'] = pd.to_numeric(rfm['Recence'], errors='coerce')
        ...
        st.write(f"**Nombre de clients analysés:** {len(rfm)}")
...
if uploaded_file:
    ...
    # Afficher les premières lignes pour aider à l'identification des colonnes
    with st.expander("Aperçu des données", expanded=True):
        st.write("**Utilisez cet aperçu pour identifier les colonnes nécessaires aux analyses**")
        st.dataframe(df.head(3))
    ...
    # Vérifier si les données nettoyées sont disponibles
    if 'df_clean' not in st.session_state:
        st.info("Veuillez nettoyer les données pour continuer")
    else:
    ...
    <div class="guide-box">
        ...
        <div class="info-card">
            <b>Conseil pour l'analyse RFM:</b> Votre jeu de données doit contenir des informations sur:
            <ul>
                <li>La date des transactions ou le nombre de jours depuis le dernier achat</li>
                <li>Le nombre d'achats par client</li>
                <li>Le montant dépensé par client</li>
            </ul>
        </div>
    </div>
    """, unsafe_allow_html=True)
